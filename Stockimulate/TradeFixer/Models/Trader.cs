...
namespace TradeFixer.Models
{
    internal class Trader
    {
        ...
        //Lazy load
        private Team _team;
        internal Team Team => _team ?? (_team = DataAccess.Instance.GetTeam(_teamId));

        //Lazy load
        private Dictionary<string, Account> _accounts;
        internal Dictionary<string, Account> Accounts => _accounts ?? (_accounts = DataAccess.Instance.GetAccounts(Id));
